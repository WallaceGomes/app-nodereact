...
function Main({ navigation }) {
    ...
    useEffect(() => {
       async function loadInitialPosition() {
        const { granted } = await requestPermissionsAsync(); //desestruturação

        if (granted) {
            const {coords} = await getCurrentPositionAsync({ //desestruturação
    ...
    return (
        <>
            <MapView onRegionChangeComplete={handleRegionChanged} initialRegion={currentRegion} style={styles.map}>
                {devs.map(dev => ( //para cara dev que aparece no mapa carrega as informações
                    <Marker
                    key={dev._id}
                    coordinate={{
